...
![Cumulative spend per agent step for the same task: a healthy run plateaus when it finishes, while a runaway loop keeps climbing.](assets/token-leak.png)

*The same task, run twice. A healthy run plateaus when it completes. A runaway loop keeps spending, and if nothing stops it, the invoice is where you find out. Illustrative.*
