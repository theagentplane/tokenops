...
def _to_lc_messages(messages: Sequence[dict[str, str]]) -> list[BaseMessage]:
...
def _from_lc_messages(messages: Sequence[BaseMessage]) -> list[dict[str, str]]:
        ...
        **_kwargs: Any,
        ...
        if max_output_tokens is not None and hasattr(chat, "bind"):
            try:
                ...
                bound = chat
        response = bound.invoke(_to_lc_messages(messages))
...
class GovernedChatModel(BaseChatModel):
    ...
    def __init__(self, complete_fn: Callable[..., Any], *, provider: str, model: str, **kwargs: Any) -> None:
    ...
    def _generate(
        ...
        **kwargs: Any,
        ...
        dicts = _from_lc_messages(messages)
