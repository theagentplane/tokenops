...
from tokenops.control import install_crossing_hook, with_governance_errors
...
def build_app():
    ...
    async def handler(payload: dict, headers: Mapping[str, str]) -> dict:
        ...
        return task_response(
            RunResult(findings=findings, summary=summary, steps=steps, token_usage=token_usage)
        )
