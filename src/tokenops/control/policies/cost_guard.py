...
class CostGuardDetector(Detector):
    ...
    def __init__(self, budget: Budget, *, threshold: float = 0.8,
                 velocity_micros_per_step: int | None = None, velocity_m: int = 5) -> None:
        self.budget = budget
    ...
    def observe(self, attr: Attribution, step: BoundaryStep, view: LedgerView) -> Signal | None:
        ...
        if sk is None:
            return None
        spent = self.budget.limit_micros - view.budget_left(self.budget.budget_id, sk, self.budget.period)
        ratio = spent / self.budget.limit_micros
        vel = view.velocity(attr.run_id, self.velocity_m)
        tripped_ratio = ratio >= self.threshold
        tripped_vel = self.velocity_micros_per_step is not None and vel >= self.velocity_micros_per_step
...
def build(budget: Budget, *, threshold: float = 0.8, mode: Mode = "minimize",
          downgrade_to: str | None = None, velocity_micros_per_step: int | None = None,
          velocity_m: int = 5) -> tuple[Detector, Policy]:
    return (
        CostGuardDetector(budget, threshold=threshold,
                          velocity_micros_per_step=velocity_micros_per_step, velocity_m=velocity_m),
