...
from tokenops.control.models import RunRegistration, RunRecord
...
def run_simulation(
    ...
    *,
    ...
    research_gov = _make_trace_governor(store, "research", log, price)
        ...
        with governance_scope(
            ...
            try:
                ...
                with downstream_run_scope(store, headers=headers, service="summarize"):
                    ...
                    summarize_gov = _make_trace_governor(store, "summarize", log, price)
...
def _make_trace_governor(
    store: ControlStore, agent: str, log: _TraceLog, price
) -> _TraceGovernor:
