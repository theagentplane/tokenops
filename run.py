...
def main() -> int:
    ...
    cfg = load_config()
    for port in {CONTROL_PLANE_PORT, cfg.research.port, cfg.summarize.port, UI_PORT}:
        _free_port(port)

    os.environ.setdefault("TOKENOPS_CONTROL_PLANE_URL", CONTROL_PLANE_URL)

    print("Starting control plane...")
    _start_server("tokenops.servers.control_plane")
