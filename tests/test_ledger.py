...
def test_pricing_and_cum_spent_single_source():
    ...
    s1 = ledger.record(Observation(attr=attr, node_type="llm", boundary_id="chat", ts=1.0,
...
def test_tool_and_delegate_costs():
    ...
    ledger.record(Observation(attr=attr, node_type="llm", boundary_id="chat", ts=1.0,
                              provider="openai", model="gpt-4o-mini", usage=Usage(input=100, output=10)))
    before = ledger.cost_micros("run-1")
    ledger.record(Observation(attr=attr, node_type="tool", boundary_id="search", ts=2.0,
                              signature="sig", result_hash="rh"))
    assert ledger.cost_micros("run-1") == before, "tool call must not move spend"
    ledger.record(Observation(attr=attr, node_type="delegate", boundary_id="d", ts=3.0,
                              rolled_up_cost_micros=25_000))
...
def test_velocity_and_recent():
    ...
    ledger.open_run("run-1")
    for i in range(1, 4):
        ledger.record(Observation(attr=attr, node_type="llm", boundary_id="chat", ts=float(i),
                                  provider="openai", model="gpt-4o-mini", usage=Usage(input=100, output=0)))
...
def test_budget_left_real_cap_and_unlimited_run_total():
    ...
    ledger.open_run("run-1")
    ledger.record(Observation(attr=attr, node_type="llm", boundary_id="chat", ts=1.0,
                              provider="openai", model="gpt-4o-mini", usage=Usage(input=1000, output=0)))
    assert ledger.budget_left("run_llm_cap", segment_key(attr, cap)) == 1_000_000 - 10_000
    assert ledger.budget_left("__run_total__", segment_key(attr, RUN_TOTAL_BUDGET)) == UNLIMITED_LEFT


def test_inflight_admit_complete_floored():
    ledger, _ = _ledger()
    ledger.admit("run:run-1"); ledger.admit("run:run-1")
...
def test_fail_closed_on_unknown_model():
    ...
    with pytest.raises(ValueError):
        ledger.record(Observation(attr=attr, node_type="llm", boundary_id="x", ts=1.0,
                                  provider="openai", model="mystery", usage=Usage(input=1)))
