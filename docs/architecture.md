...
Control plane under `src/tokenops/control/`; HTTP service at `src/tokenops/control_plane/`.
Agents and UIs call `open_store()` — remote HTTP when split, direct SQLite in unit tests.
...
|-------------|--------|---------|
| Control-plane API | `control_plane/app.py` :8000 | sole SQLite owner when `TOKENOPS_CONTROL_PLANE_URL` is set |
| `open_store()` | `control/store_factory.py` | `RemoteStore` (HTTP) or `SqliteStore` (local/tests) |
| `POST /v1/runs` + `POST /v1/tasks` | control plane + `a2a/server.py` | register run dims; task requires `X-TokenOps-Run-Id` |
...
graph TD
    ...
    subgraph cp["control-plane service :8000"]
        api["control_plane/app.py"]
        sqlite[("SQLite tokenops.db")]
        api --> sqlite
    end
    ...
    subgraph control["control plane library"]
    ...
    admin -->|open_store| store
    store -->|HTTP| api
    agent -->|open_store| store
    agent -->|write RunRecord| store
    store -->|list_runs| dash
    api -->|governance_config_for| cfg
    boundary --> engine
    agent --> boundary
