...
| Job | Status | Where |
|---|---|---|
| Control-plane API layer (split mode) | ✅ | `control_plane/app.py` :8000; agents/UIs use `open_store()` → `RemoteStore` |
| Run registration (`intent`, `user_dims`) | ✅ | `POST /v1/runs` on control plane (local fallback: research `a2a/server.py`) |
...
```
POST /v1/runs  →  control plane :8000  →  register run_id + intent + user_dims (SQLite)
POST /v1/tasks + X-TokenOps-Run-Id  →  research/summarize agents
  → open_store() → RemoteStore (or local SqliteStore in tests)
